...
pub fn run() {
    ...
    // — and, critically, before **any** thread spawns: env mutation isn't thread-safe.
    // No-op on macOS/Windows and on healthy AMD/Intel Mesa stacks; the user's own env
    // and `RECUE_DISABLE_DMABUF` override it. See `linux_webkit`.
