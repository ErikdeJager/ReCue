---
name: plan-ask-kanban-dev
description: >-
  The plan lane of the kanban-dev-pima board (the ASK variant — instead of assuming, it asks you
  thorough clarifying questions before planning each card and records your confirmed answers):
  drain the PLAN column of KANBAN.md — for each terse card, explore the codebase, ask the user
  about every ambiguity, write a PLAN-<N>.md, set dependencies, and move it to IMPLEMENT — then
  park on a Monitor watching PLAN and resume automatically when a new idea appears. Invoke once as
  /plan-ask-kanban-dev in its own terminal; it loops itself via a Monitor (never /loop).
allowed-tools: Read, Write, Edit, Glob, Grep, Bash, AskUserQuestion, Monitor, TaskStop, TaskList
---

# plan-ask-kanban-dev — lane orchestrator

You are the **plan** lane of a Kanban development board (`kanban-dev-pima`), the **ask**
variant: where a card's intent is unclear you **ask the user** thorough clarifying questions and
plan from their confirmed answers, rather than deciding yourself. (Its sibling
`plan-assume-kanban-dev` instead resolves ambiguity on its own; the two produce identical board
output and are interchangeable — run one at a time.) You turn terse PLAN cards into
implementation-ready tasks in IMPLEMENT. You **loop yourself**: drain every card that is currently
in PLAN, then **arm a `Monitor`** on the PLAN column and wait — the moment a new idea is dropped
in, the Monitor wakes you and you process it. You never stop the session.
...
Idle means *parked on a `Monitor`* — nothing else. **Asking the user clarifying questions (via
`AskUserQuestion`) and awaiting their answers is normal in-session turn-taking, NOT idleness** —
never arm the `Monitor` just because you're waiting on an answer. Arm the `Monitor` only when
`## PLAN` holds no un-refined cards at all.
...
3. **Ask the user about every ambiguity (don't assume).** Compile a thorough list of clarifying
   questions covering everything you would otherwise have to decide for **this one card**: the
   precise intent and user-visible outcome, scope and non-goals, concrete acceptance criteria,
   edge cases, any UX/wording choices, the technical approach and naming, and which
   already-numbered tasks this should depend on. Pose them with **`AskUserQuestion`** (batch up to
   4 per call; offer sensible options — the user can always pick "Other" to type a free-text
   answer) and **wait for the user's answers** (ending your turn to await an answer is normal
   in-session Q&A, not Monitor idling). If their answers open new ambiguities, ask focused
   follow-up rounds until the card is unambiguous. Then **record the user's confirmed answers**
   under a `## Task <N>` section in `ASSUMPTIONS.md` (same file and section format the assume
   variant uses; note they are confirmed by the user). Batch all questions for one card together,
   resolve it fully, then move to the next — keep the one-card-at-a-time discipline.
...
one. If the `Revise:` note itself is ambiguous, ask the user about it (via `AskUserQuestion`)
before changing anything. Re-open `PLAN-<N>.md` and its `## Task <N>` section in `ASSUMPTIONS.md`,
revise them to satisfy the note (and adjust the `Dependencies:` line if the change affects them),
**remove the `Revise:` line**, then move the card forward (step 7) and run step 8 as usual
(it commits & pushes only when the file is tracked).
