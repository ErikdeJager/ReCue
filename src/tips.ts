...
/** Mac chord token → Windows/Linux form: ⌘→Ctrl+, a **leading** ⌥→Alt+ (the
 * keybind rework's ⌥1/⌥2/⌥3 view chords), ⇧→Shift+, a modifier ⌥→Alt+, ⏎→Enter.
 ...
 * macOS (and before the platform loads); mac chords become their Ctrl+/Alt+ form
 * on Windows AND Linux. */
export function renderTip(platform: string, tip: string): string {
  if (!(isWindows(platform) || isLinux(platform))) return tip;
  return tip.replace(CHORD_RE, (_m, lead: string, mod: string, key: string) => {
    const l = lead === "⌥" ? "Alt+" : "Ctrl+";
    const m = mod === "⇧" ? "Shift+" : mod === "⌥" ? "Alt+" : "";
    return `${l}${m}${key === "⏎" ? "Enter" : key}`;
  });
}
