 ...
 * same rendering is used everywhere. Since task 437 a PTY renders in ANY window
 * that shows it (426/427 mirroring — the old #84 single-owner guard is gone).
 *
 * One "render in one place" guard keeps a pooled terminal / single auto-save hook
 * from mounting twice **within a window**:
...
function ItemContent({
  ...
  if (content.kind === "filetree" && content.repoPath) {
    // Stateless repo data (#167), like diff — no PTY guard needed.
    return (
      <PanelSuspense>
